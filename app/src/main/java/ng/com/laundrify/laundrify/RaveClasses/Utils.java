...
public class Utils {
    ...
    public static boolean wasTxSuccessful(RavePayInitializer ravePayInitializer, String responseAsJSONString) {
        ...
        try {
            ...
            if (areAmountsSame(amount, txAmount) &&
                    chargeResponse.equalsIgnoreCase("00") && (status.contains("success") | status.contains("pending-capture")) &&
    ...
    //Seyi 2018
    public static boolean wasCardTxSuccessful(RavePayInitializer ravePayInitializer, String responseAsJSONString) {

        String amount = ravePayInitializer.getAmount() + "";
        String currency = ravePayInitializer.getCurrency();
        String transaction_ref = ravePayInitializer.getTxRef();
        ...
        try {
            ...
            String txAmount = jsonData.getString("amount");
            String txCurrency = jsonData.getString("currency");
            String txRef = jsonData.getString("txref");

            if (areAmountsSame(amount, txAmount) &&
                    txRef.equalsIgnoreCase(transaction_ref) && (status.contains("success") | status.contains("pending-capture")) &&
                    currency.equalsIgnoreCase(txCurrency)) {
                Log.d("RAVE TX V", "true");
                return true;
            }
